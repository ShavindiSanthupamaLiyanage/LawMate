...
public class GetInactiveUsersQuery : IRequest<List<UserDetailResponseDto>>;

public class GetInactiveUsersQueryHandler
    : IRequestHandler<GetInactiveUsersQuery, List<UserDetailResponseDto>>
{
    ...
    public async Task<List<UserDetailResponseDto>> Handle(
        GetInactiveUsersQuery request,
        CancellationToken cancellationToken)
    {
        return await _context.USER_DETAIL
            .Where(x => (int)x.State == 2)
