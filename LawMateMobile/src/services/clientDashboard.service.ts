...
export class ClientDashboardService {
    static async getDashboardHome(): Promise<ClientDashboardHomeResponse> {
        ...
        const response = await fetch(`${API_CONFIG.BASE_URL}${ENDPOINTS.CLIENT.DASHBOARD_HOME}`, {
            method: 'GET',
            headers: {
    ...
    static async getDashboardActivity(): Promise<ClientActivity[]> {
        ...
        const response = await fetch(`${API_CONFIG.BASE_URL}${ENDPOINTS.CLIENT.DASHBOARD_ACTIVITY}`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${token}`,
                Accept: 'application/json',
